...
import com.xianyusmart.mapper.XianyuHumanInterventionRecordMapper;
 ...
 * 人工接管管理器
 *
 * <p>数据库保存人工接管状态，ConcurrentHashMap 仅作为热点缓存。</p>
 *
 * <h3>核心机制：</h3>
 * <ul>
...
public class HumanTakeoverManager {
    ...
    public void takeover(Long accountId, String sId, int minutes) {
        takeover(accountId, null, sId, minutes);
    }
    ...
    public void takeover(Long accountId, String xyGoodsId, String sId, int minutes) {
        ...
        record.setSId(sId);
        record.setEndTime(LocalDateTime.ofInstant(
                java.time.Instant.ofEpochMilli(expireTime), ZoneId.systemDefault()));
    ...
    public boolean isTakenOver(Long accountId, String sId) {
        ...
        if (expireTime == null) {
            XianyuHumanInterventionRecord record = interventionRecordMapper.findActiveByAccountAndSId(accountId, sId);
            if (record == null) {
                return false;
            }
            expireTime = record.getEndTime().atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
            takeoverMap.put(key, expireTime);
        }
        if (now > expireTime) {
