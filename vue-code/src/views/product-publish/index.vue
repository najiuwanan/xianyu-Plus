...
const publishBlockers = computed(() => {
  ...
  if (!selectedAccountId.value) blockers.push('请选择发布账号')
  if (!schema.value) blockers.push('请重新识别类目')
  else {
    if (schema.value.supportLevel !== 'GENERAL_FORM') blockers.push(schema.value.supportLabel || '当前类目暂不支持通用发布')
    if (schema.value.dependentPropertyCount > 0) blockers.push(`还有 ${schema.value.dependentPropertyCount} 个联动属性未加载`)
    if (!requiredPropertiesReady.value) blockers.push('请填写全部必填属性')
  }
  if (!images.value.length) blockers.push('请上传商品图片')
  if (!selectedLocationKey.value) blockers.push('请选择发布地点')
  if (form.title.trim().length < 2) blockers.push('请填写商品标题')
  if (form.description.trim().length < 2) blockers.push('请填写商品描述')
  if (!(form.price > 0)) blockers.push('售价必须大于 0')
  if (!acknowledged.value) blockers.push('请勾选发布确认')
  if (confirmation.value !== '确认发布') blockers.push('请输入“确认发布”')
  return blockers
})
const canPublish = computed(() => publishBlockers.value.length === 0 && !publishing.value)
...
const prepareBatch = async () => {
  ...
  for (const state of states) {
    try {
      ...
      if (!accountSchema || accountSchema.supportLevel !== 'GENERAL_FORM' || accountSchema.dependentPropertyCount > 0) {
...
<template>
  <main class="publish-page">
    ...
    <section class="publish-card confirm-card">
      ...
      <p v-if="!batchMode && publishBlockers.length" class="publish-blockers"><strong>还需完成：</strong>{{ publishBlockers.join('；') }}</p>
    </section>
  </main>
</template>
